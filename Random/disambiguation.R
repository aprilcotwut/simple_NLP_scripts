...
for (pair in word_pairs) {
  ...
  params <- list(learn_rate = c(0.05, 0.1, 0.2),
                 max_depth = c(3, 10))
  # Call grid search
  ...
  # # # Step 2.4 Commence GLM

  # save grid search
  grid_name <- paste0("glm_", pair_name)

  # NN hyper parameters
  params <- list(alpha = c(0, 0.2, 0.5, 0.7))

  # Choose top performing by accuracy
  grid <- h2o.grid("glm", y = "target",
                          grid_id = grid_name,
                          training_frame = train.hex,
                          validation_frame = test.hex,
                          family = "binomial",
  ...
  # # # Step 2.5 Commence Naive Bayes double check

  m <- h2o.naiveBayes(y = "target",
                      training_frame = train.hex,
                      validation_frame = test.hex,
                      )

  sink("nbayes_results.txt", append = T)
  cat('\n')
  h2o.confusionMatrix(m, test.hex)
  sink(NULL)
